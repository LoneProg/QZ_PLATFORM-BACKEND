...
const getTestQuestion = asyncHandler(async (req, res) => {
  ...
  if (!testResult) return res.status(404).json({ message: 'Test not started or does not exist' });

  if (testResult.status !== 'in-progress') return res.status(400).json({ message: 'Test already submitted or not in progress' });
...
const submitTest = asyncHandler(async (req, res) => {
  ...
  // Calculate score
  const correctAnswersCount = testResult.answers.filter(answer => answer.isCorrect).length;
  ...
  res.status(200).json({ message: 'Test submitted successfully', score: testResult.score });
});
