...
const getDashboardOverview = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const totalScheduledTests = await Test.countDocuments({ createdBy: userId, 'scheduling.status': 'scheduled' });
        const totalFinishedTests = await Test.countDocuments({ createdBy: userId, 'scheduling.status': 'finished' });
        const totalInProgressTests = await Test.countDocuments({ createdBy: userId, 'scheduling.status': 'active' });
        ...
        res.status(500).json({ message: 'Failed to retrieve dashboard overview', error });
    }
});
...
const getRecentlyCreatedTests = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to retrieve recently created tests', error });
    }
});
...
const getRecentlyScheduledTests = asyncHandler(async (req, res) => {
    ...
    try {
        const recentScheduledTests = await Test.find({ createdBy: userId, 'scheduling.status': 'scheduled' })
        ...
        res.status(500).json({ message: 'Failed to retrieve recently scheduled tests', error });
    }
});
...
const getRecentTestTakers = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Failed to retrieve recent test taker tracker list', error });
    }
});
...
const getUpcomingTests = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to retrieve upcoming tests', error });
    }
});
...
const getTestPerformanceSummary = asyncHandler(async (req, res) => {
    ...
    try {
        const performanceSummary = await Test.aggregate([
            { $match: { createdBy: userId } },
            {
                $group: {
                    ...
                    passRate: { $avg: { $cond: [{ $gte: ["$results.score", "$passingScore"] }, 1, 0] } }
                }
            }
        ]);
        ...
        res.status(500).json({ message: 'Failed to retrieve performance summary', error });
    }
});
